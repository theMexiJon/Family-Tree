    ...
    <form action={addPerson} className="mt-5 flex flex-col gap-4">
      ...
      {userName ? (
        <>
          ...
          <p className="text-xs text-[--color-ink-faint]">
            {t('addingAs', locale)} <span className="font-medium text-[--color-ink-muted]">{userName}</span>
          </p>
        </>
      ) : (
        <div className="flex flex-col gap-1">
          <label htmlFor="added_by" className={LABEL}>
            {t('yourName', locale)} <span className="text-[--color-accent]">*</span>
          </label>
          <input id="added_by" name="added_by" type="text" required maxLength={80} placeholder="e.g. Sarah Johnson" className={INPUT} />
        </div>
      )}

      <div className="flex flex-col gap-1">
        <span className={LABEL}>{t('photo', locale)} <span className={OPTIONAL}>({t('optional', locale)})</span></span>
        <PhotoUpload />
      </div>

      <div className="flex flex-col gap-1">
        <label htmlFor="full_name" className={LABEL}>
          {t('theirName', locale)} <span className="text-[--color-accent]">*</span>
        </label>
        <input id="full_name" name="full_name" type="text" required maxLength={120} placeholder="e.g. Robert Johnson" className={INPUT} />
      </div>

      <div className="flex flex-col gap-1">
        <label htmlFor="maiden_name" className={LABEL}>
          {t('maidenName', locale)} <span className={OPTIONAL}>({t('optional', locale)})</span>
        </label>
        <input id="maiden_name" name="maiden_name" type="text" maxLength={120} placeholder="e.g. Williams" className={INPUT} />
      </div>

      <div className="flex flex-col gap-1">
        <span className={LABEL}>{t('birthday', locale)} <span className={OPTIONAL}>({t('optional', locale)})</span></span>
        <div className="flex gap-2">
          ...
          <input name="birth_year" type="number" min={1800} max={new Date().getFullYear()} placeholder={locale === 'es' ? 'Año' : 'Year'} className={`w-24 ${INPUT}`} />
        </div>
      </div>
        ...
        <>
          ...
          {isDeceased && (
            <div className="flex flex-col gap-1 pl-5">
              <span className={LABEL}>{t('dateOfPassing', locale)} <span className={OPTIONAL}>({t('optional', locale)})</span></span>
              <div className="flex gap-2">
                ...
                <input name="death_year" type="number" min={1800} max={new Date().getFullYear()} placeholder={locale === 'es' ? 'Año' : 'Year'} className={`w-24 ${INPUT}`} />
              </div>
            </div>
          )}
        </>
      ...
      <div className="flex flex-col gap-1">
        <label htmlFor="branch" className={LABEL}>
          {t('familyBranch', locale)} <span className={OPTIONAL}>({t('optional', locale)})</span>
        </label>
        <input id="branch" name="branch" type="text" maxLength={80} placeholder={t('familyBranchPlaceholder', locale)} className={INPUT} />
      </div>

      <div className="flex flex-col gap-1">
        <label htmlFor="bio" className={LABEL}>
          {t('shortBio', locale)} <span className={OPTIONAL}>({t('optional', locale)})</span>
        </label>
        <textarea id="bio" name="bio" rows={2} maxLength={500} placeholder={t('bioPh', locale)} className={INPUT} />
      </div>
      ...
      {people.length > 0 && (
        <div>
          ...
          >
            ...
            {showRelSection ? t('removeRel', locale) : t('linkExisting', locale)}
          </button>
        </div>
      )}
      ...
      {people.length > 0 && showRelSection && (
        <div className="flex flex-col gap-3 rounded-xl border border-[--color-paper-dark] p-4">
          <div className="flex flex-col gap-1">
            <label htmlFor="related_to" className={LABEL}>{t('relatedTo', locale)}</label>
            <select id="related_to" name="related_to" value={relatedTo} onChange={e => setRelatedTo(e.target.value)} className={`w-full ${SELECT}`}>
              <option value="">{t('selectPerson', locale)}</option>
          ...
          {relatedTo && (
            <>
              <div className="flex flex-col gap-2">
                <span className={LABEL}>{t('relType', locale)}</span>
              ...
              {relType === 'partner' && (
                <>
                  <div className="flex flex-col gap-2">
                    <span className={LABEL}>{t('relStatus', locale)}</span>
                    <div className="flex gap-4">
                      {(['married', 'partners', 'divorced'] as const).map(s => (
                        <label key={s} className="flex cursor-pointer items-center gap-1.5 text-sm text-[--color-ink]">
                          ...
                          {t(s as 'married' | 'partners' | 'divorced', locale)}
                        </label>
                      ))}
                    </div>
                  </div>
                  <div className="flex flex-col gap-1">
                    <span className={LABEL}>{t('weddingDate', locale)} <span className={OPTIONAL}>({t('optional', locale)})</span></span>
                    <div className="flex gap-2">
                      ...
                      <input name="wedding_year" type="number" min={1800} max={new Date().getFullYear()} placeholder={locale === 'es' ? 'Año' : 'Year'} className={`w-24 ${INPUT}`} />
                    </div>
                  </div>
                </>
              )}
              {relType === 'parent_child' && (
                <div className="flex flex-col gap-2">
                  <span className={LABEL}>{t('personIsThe', locale)}</span>
      ...
      <SubmitButton label={t('addToTree', locale)} />
    </form>
  )
}
